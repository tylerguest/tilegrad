...
def run(kernel, *tensors, realize: bool=True):
  ...
  if not isinstance(out, Tensor): raise TypeError("first tensor must be the output Tensor")
  for i, t in enumerate(inputs):
    if not isinstance(t, Tensor): raise TypeError(f"input tensor {i} is not a Tensor")
